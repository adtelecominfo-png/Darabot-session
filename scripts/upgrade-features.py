...
html = html.replace('''        <div class="feature-strip"><span><b>✓</b> No session data stored</span><span><b>✓</b> Copy-ready output</span><span><b>✓</b> Works on mobile</span></div>''', '''        <div class="feature-strip"><span><b>✓</b> No session data stored</span><span><b>✓</b> Copy-ready output</span><span><b>✓</b> Works on mobile</span></div>
        <div class="install-banner" id="installBanner"><span>Install DARATECH BOT V2 for faster access.</span><button onclick="installApp()">Install app</button></div>
...
html = html.replace('''    let qrSessionKey = null, qrPollTimer = null, qrEncoded = '';
    ...
    const translations = { en: { hero: 'WhatsApp connection made simple', copy: 'Generate a secure DARATECH BOT V2 session ID in seconds. Choose the connection method that works best for you.' }, fr: { hero: 'Connexion WhatsApp simplifiée', copy: 'Générez votre identifiant de session DARATECH BOT V2 en quelques secondes.' }, ha: { hero: 'Haɗin WhatsApp cikin sauƙi', copy: 'Ƙirƙiri session ID na DARATECH BOT V2 cikin daƙiƙu.' } };''')
